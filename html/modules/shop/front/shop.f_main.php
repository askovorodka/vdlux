...
SWITCH (TRUE) {
	...
	CASE ($url[$n]=='submit' && $url[$n-1]=='basket' && count($url)==3):
		...
		if (!empty($email))
		{
			...
			Mail::send_mail($email,"noreply@".$_SERVER['SERVER_NAME'],"Новый заказ в интернет магазине",$body,'','html','standard','Windows-1251');
		}


		$admin_body=$smarty->fetch($templates_path.'/admin_order_notice.txt');
		Mail::send_mail(SEND_ORDER_TO,"noreply@".$_SERVER['SERVER_NAME'],"Новый заказ в интернет магазине",$admin_body, $attach,'html','standard','WIndows-1251');
